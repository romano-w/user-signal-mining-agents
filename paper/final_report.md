...
We investigate whether decomposing a founder's natural-language question into structured sub-tasks—intent decomposition, multi-query retrieval, grounded synthesis, and evidence verification—produces more actionable focus points from customer reviews than a single-step zero-shot baseline. Using an LLM-as-judge evaluation across 20 prompts spanning three domains (restaurants, SaaS, ecommerce), we find that the multi-step pipeline is preferred on 17 of 20 prompts, with overall preference improving from 3.50 to 4.65 on a 5-point scale (+1.15). Gains are largest on groundedness (+0.65) and relevance (+0.80), confirming that explicit retrieval planning and evidence grounding reduce hallucination and improve specificity. Human annotation (n=20, 1 annotator) shows 70% exact agreement with the LLM judge on winner selection (Cohen's κ = 0.34), suggesting moderate but imperfect alignment. We release the full system, evaluation framework, and dataset pipeline as open-source software.
...
One annotator scored all 20 blinded A/B tasks. Exact agreement with the LLM judge on winner selection is 70% (Cohen's κ = 0.34, fair agreement). The judge and human agree on the direction of preference in most cases, though the human annotator assigns narrower score gaps, suggesting the LLM judge is more decisive. This moderate agreement is consistent with findings in the LLM-as-judge literature (Zheng et al., 2023).
...
**Limitations.** Our evaluation uses a single LLM as both generator and judge, risking self-preference bias. The retrieval benchmark uses only 2 labeled queries—too sparse for confident IR evaluation. Human annotation is limited to one annotator, precluding interannotator agreement analysis. Domain transfer uses restaurant data with non-restaurant prompts, which is a deliberate stress test but not a realistic deployment scenario.
